...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	if function == "initMarble" { //create a new marble
		return t.initMarble(stub, args)
	} else if function == "readMarble" { //change owner of a specific marble
		return t.readMarble(stub, args)
	}
...
func (t *SimpleChaincode) initMarble(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	objectType := "marble"
	var marble myMarble.Marble
	marble.ObjectType = objectType
	marble.Name = marbleName
	marble.Color = color
	marble.Size = size
	marble.Owner = owner
	// marble := &Marble{, , color, size, owner}
	marbleJSONasBytes, err := proto.Marshal(marble)
	if err != nil {
	...
	err = setChaincodeEvent(stub, "anEvent", "Marble", "init", marbleName, marbleJSONasBytes)
	if err != nil {
		return shim.Error("Couldnt set EVent " + err.Error())
	}
	return shim.Success(nil)
}

func setChaincodeEvent(stub shim.ChaincodeStubInterface, eventName string, assetType string, action string, assetAddress string, payloadData []byte) error {
	newPayload := payload{}
	newPayload.AssetType = assetType
	newPayload.Action = action
	//newPayload.User = users
	newPayload.AssetAddress = assetAddress
	newPayload.PayloadData = payloadData
	payloadAsBytes, err := json.Marshal(newPayload)
	if err != nil {
		return shim.Error("Marshall Error " + err.Error())
	}
	err = stub.SetEvent(eventName, payloadAsBytes)
	if err != nil {
		return shim.Error("Set event Error " + err.Error())
	}
	return nil
}
